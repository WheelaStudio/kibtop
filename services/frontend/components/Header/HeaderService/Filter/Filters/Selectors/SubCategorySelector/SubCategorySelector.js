...
const SubCategorySelector = ({ holder, name, items, disabled, zIndex }) => {
  ...
  useEffect(() => {
    if (!!scrollElement.current && scrollElement.current.offsetHeight > 340)
      scrollElement.current.style.borderRadius = "0";
  }, [scrollElement.current?.offsetHeight]);
  return (
    <>
      <div
        className={adaptiveSelect}
        style={isOpen ? { ...selctedStyle, marginTop: 12 } : { marginTop: 12 }}>
