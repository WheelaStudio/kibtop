...
import {
  addAdvertThunk,
  setAddAdvertCategory,
  setAddAdvertFormStep,
  setNewAdvertData,
} from "../../../store/slices/AddAdvertSlice";
...
const AddAdvertFormContainer = () => {
  const {
    formStep,
    // category,
    newAdvert: { advertId, advertCategory },
  } = useSelector(state => state.addAdvert);
  ...
  const switchFormStepToStart = () => dispatch(setAddAdvertFormStep(4));
  ...
  return (
    <AddAdvertForm
      {...{ formStep, onAddAdvertSubmit, category, switchFormStepToStart }}
    />
  );
};
